...
- [15.1](#testing--data-test-id-name) Для целей тестирования используйте `data`-атрибут `testid` (т.е. `data-testid`).
  ...
  Можно также использовать и другие названия, например, `test-id`. При этом понадобится дополнительная настройка `testing-library`. Не очень важно само название. Важно, чтобы в рамках проекта должно использоваться только одно название `data`-атрибута для тестирования.
